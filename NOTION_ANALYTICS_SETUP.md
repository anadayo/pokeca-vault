# POKÉCA VAULT 日次レポート

毎朝9:15（JST）に前日のGA4と価格更新状況をNotionへ保存します。POKÉCA VAULT、OP CARD VAULT、TAG TOKYOは別々のNotionデータベースへ保存し、同じサービス・日付を再実行した場合はその行を更新します。

日次タイトルは `2026-09-18（金）｜POKÉCA VAULT` 形式です。日付だけを重複判定に使うため、同じ日の行は増えません。同期日時はJSTで保存します。

## Notionデータベース

以下のプロパティ名・種類でデータベースを作成します。

| プロパティ | 種類 |
| --- | --- |
| 日次レポート | タイトル |
| 日付 | 日付 |
| 月 | セレクト |
| アクティブユーザー | 数値 |
| 新規ユーザー | 数値 |
| セッション | 数値 |
| PV | 数値 |
| サイト計測クリック | 数値 |
| メルカリ公式クリック | 数値（手入力） |
| クリック差分 | 数式 `prop("サイト計測クリック") - prop("メルカリ公式クリック")` |
| CTR | 数値（表示形式をパーセント） |
| カード詳細表示 | 数値 |
| 検索回数 | 数値 |
| Xシェア | 数値 |
| 価格データ日 | 日付 |
| 価格更新 | セレクト |
| サイト | URL |
| 同期日時 | 日付 |

両データベースに「月次確認」ビューを用意し、`月`でグループ化します。PV・ユーザー・クリックは合計、CTRは平均を表示すると月ごとの比較ができます。

TAG TOKYO用データベースはカード系と分け、以下のプロパティを作成します。

| プロパティ | 種類 |
| --- | --- |
| 日次レポート | タイトル |
| 日付 | 日付 |
| 月 | セレクト |
| アクティブユーザー | 数値 |
| 新規ユーザー | 数値 |
| セッション | 数値 |
| PV | 数値 |
| TAG ON開始 | 数値 |
| CROSS表示 | 数値 |
| TAG送信 | 数値 |
| MATCH成立 | 数値 |
| トーク送信 | 数値 |
| 通報 | 数値 |
| サイト | URL |
| 同期日時 | 日付 |

## GitHub Secrets

リポジトリの Settings > Secrets and variables > Actions に以下を登録します。

- `NOTION_TOKEN`: Notionインテグレーションのトークン
- `GA_SERVICE_ACCOUNT_JSON`: Google Cloudサービスアカウント鍵のJSON全文

日次レポートのデータソースIDはワークフローへ設定済みです。

- POKECA VAULT: `3d7c9239-b3c9-802c-8506-000b712815e5`
- OP CARD VAULT: `077c9239-b3c9-82fb-b8bf-87cf83077643`

Notionでは3つの対象データベースの「接続」からインテグレーションを追加します。Google CloudではAnalytics Data APIを有効にし、サービスアカウントのメールアドレスをGA4プロパティ `552987217` の閲覧者として追加します。

TAG TOKYOは `TAG TOKYO｜運用・アクセス日次レポート` という表名からデータソースを自動検出します。ID用Secretは不要ですが、表名を変更する場合は同期スクリプトの `notionDataSourceTitle` も更新します。

## 手動実行

GitHub Actionsの「Save daily analytics to Notion」から実行できます。`report_date`を指定すると過去日の再取得もできます。

`normalize_history`をオンにして実行すると、既存行のタイトル・日付・月を統一し、同じ日付の重複行を最新の1件だけ残してアーカイブします。アクセス数や手入力したメルカリ公式クリックは保持されます。

メルカリアンバサダー管理画面の公式クリック数は公開APIがないため自動取得せず、`メルカリ公式クリック`へ手入力します。サイト側のクリック数との差は`クリック差分`で確認できます。
